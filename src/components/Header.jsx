...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-black/10 bg-white/85 backdrop-blur">
      <div className="mx-auto flex max-w-page items-center gap-4 px-5 py-3">
      ...
      <nav className="border-t border-black/5">
        <ul className="mx-auto flex max-w-page items-center gap-1 overflow-x-auto px-5 py-2 text-sm">
          {categories.map((cat) => (
            <li key={cat.id}>
              <NavLink
                to={`/category/${cat.id}`}
